...
import torch
from IC import gaussian, step, disc_source, bump, hat, holhraum, lattice
...
def testing(params):
    ...
    with torch.no_grad():
        ...
        np.save("exact.npy",exact)
        # if IC_idx == 0:
        #     exact_np = np.load("linesource_37.npy")
        # elif IC_idx ==5:
        #     if T == 2:
        #         exact_np = np.load("holhraum_37_T2.npy")
        #     elif T==4:
        #         exact_np = np.load("holhraum_37_T4.npy")
        ...
        error0     = torch.sqrt(obj_func(PN  - exact[:,:,:,:num_basis])/obj_func(exact[:,:,:,:num_basis]))
        errorf     = torch.sqrt(obj_func(FPN -  exact[:,:,:,:num_basis])/obj_func(exact[:,:,:,:num_basis]))
        flux_err0  = torch.sqrt(obj_func(PN[:,:,:,0]  - exact[:,:,:,0])/obj_func(exact[:,:,:,0]))
        flux_errf  = torch.sqrt(obj_func(FPN[:,:,:,0] - exact[:,:,:,0])/obj_func(exact[:,:,:,0]))

    total_error_reduction = errorf/error0
    flux_error_reduction  = flux_errf/flux_err0

    print(f'TOTAL:  P{N} error = ', error0,      f'FP{N} error = ', errorf ,     'total error_reduction = ', total_error_reduction)
    print(f'FLUX:   P{N} error = ', flux_err0,   f'FP{N} error = ', flux_errf ,  'flux error_reduction  = ', flux_error_reduction)
    ...
    ax_PN.tick_params(axis='x', which='both', bottom=False, top=False, labelbottom=False)
    ...
    ax_FPN.tick_params(axis='x', which='both', bottom=False, top=False, labelbottom=False)
    ...
    fig_exact, ax_exact = plt.subplots(figsize=(6, 4.5), constrained_layout=True)
    plt.rcParams.update({'font.size': 16})
    contour_exact = ax_exact.contourf(y, x, exact, levels, cmap=cmap)
    ax_exact.set_title(f"$P_{{{N_exact}}}$")
    ax_exact.tick_params(axis='x', which='both', bottom=False, top=False, labelbottom=False)
    ax_exact.tick_params(axis='y', which='both', left=False, right=False, labelleft=False)
    ...
    plt.xlim(xl,xr)
    line1, = ax_slice0.plot(x, exact[plot_idx ,:], color='r', label= f'$P_{{{N_exact}}}$')
    line2, = ax_slice0.plot(x, PN[plot_idx ,:], linestyle='--', color='b', label=f'$P_{{{N}}}$')
    line3, = ax_slice0.plot(x, FPN[plot_idx ,:], linestyle='-.', color='g', label=f'$FP_{{{N}}}$')  
    ax_slice0_sigf = ax_slice0.twinx()
    line4, = ax_slice0_sigf.plot(x, sigf[plot_idx,:], linestyle=':', color='m', label=r'$\sigma_f$')
    ...
    line2, = ax_slice45.plot(x, PN45, linestyle='--', color='b', label=f'$P_{{{N}}}$')
    line3, = ax_slice45.plot(x, FPN45, linestyle='-.', color='g', label=f'$FP_{{{N}}}$')  
    ax_slice45_sigf = ax_slice45.twinx()
    line4, = ax_slice45_sigf.plot(x, sigf[plot_idx,:], linestyle=':', color='m', label=r'$\sigma_f$')
    lines = [line1, line2, line3, line4] 
    labels = [line.get_label() for line in lines]
    ...
    ax_sig.tick_params(axis='x', which='both', bottom=False, top=False, labelbottom=False)
