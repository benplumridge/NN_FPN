...
def gaussian_training(num_x, num_y, x, y):
...
def gaussian_testing(num_x, num_y, x, y):
    # isotropic initial conditions: Don't need to include angles in initial data
    psi0 = torch.zeros([num_y + 1, num_x + 1], dtype=torch.float32)
    cx = 0
    cy = 0
...
def lattice(params):
    ...
    psi0 = torch.zeros([num_y + 1, num_x + 1])
    sigs = torch.ones([num_y + 1, num_x + 1]) * sigs_white
    sigt = torch.ones([num_y + 1, num_x + 1]) * (siga_white + sigs_white)
    source = torch.zeros([num_y + 1, num_x + 1])
    ...
    for i in range(0, num_y + 1):
        for j in range(0, num_x + 1):
            # black regions
            if (
                (y_edges[i] >= 1 and y_edges[i] <= 2)
                or (y_edges[i] >= 3 and y_edges[i] <= 4)
                or (y_edges[i] >= 5 and y_edges[i] <= 6)
            ):
                if (x_edges[j] >= 1 and x_edges[j] <= 2) or (
                    x_edges[j] >= 5 and x_edges[j] <= 6
                ):
                    sigs[i, j] = sigs_black
                    sigt[i, j] = siga_black + sigs_black
            if (y_edges[i] >= 1 and y_edges[i] <= 2) and (
                x_edges[j] >= 3 and x_edges[j] <= 4
            ):
                sigs[i, j] = sigs_black
                sigt[i, j] = siga_black + sigs_black
            if (y_edges[i] >= 2 and y_edges[i] <= 3) or (
                y_edges[i] >= 4 and y_edges[i] <= 5
            ):
                if (x_edges[j] >= 2 and x_edges[j] <= 3) or (
                    x_edges[j] >= 4 and x_edges[j] <= 5
                ):
                    sigs[i, j] = sigs_black
                    sigt[i, j] = siga_black + sigs_black
            # orange region
            if (y_edges[i] >= 3 and y_edges[i] <= 4) and (
                x_edges[j] >= 3 and x_edges[j] <= 4
            ):
                sigs[i, j] = sigs_orange
                sigt[i, j] = siga_orange + sigs_orange
                source[i, j] = source_orange
