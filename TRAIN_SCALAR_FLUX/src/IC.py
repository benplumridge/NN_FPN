...
def lattice(params):
    ...
    for i in range(0,num_y+1):
        for j in range(0,num_x+1):
            #black regions
            if(y_edges[i]>= 1 and y_edges[i]<=2) or (y_edges[i]>= 3 and y_edges[i]<=4) or (y_edges[i]>= 5 and y_edges[i]<=6):
                if(x_edges[j]>= 1 and x_edges[j]<=2)  or (x_edges[j]>= 5 and x_edges[j]<=6):
                    sigs[i,j] = sigs_black
                    sigt[i,j] = siga_black + sigs_black
            if(y_edges[i]>= 1 and y_edges[i]<=2) and (x_edges[j]>= 3 and x_edges[j]<=4):
                sigs[i,j] = sigs_black
                sigt[i,j] = siga_black + sigs_black
            if(y_edges[i]>= 2 and y_edges[i]<=3) or (y_edges[i]>= 4 and y_edges[i]<=5) : 
                if(x_edges[j]>= 2 and x_edges[j]<=3) or (x_edges[j]>= 4 and x_edges[j]<=5):
                    sigs[i,j] = sigs_black
                    sigt[i,j] = siga_black + sigs_black
            #orange region
            if(y_edges[i]>= 3 and y_edges[i]<=4) and (x_edges[j]>= 3 and x_edges[j]<=4):
