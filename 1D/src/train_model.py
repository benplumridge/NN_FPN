...
def training(params):
    ...
    psi0_edges[reg1,:]     = gaussian(num_x,x_edges)[0] 
    psi0_edges[reg2,:]     = heaviside(num_x,x_edges)[0]
    psi0_edges[reg3,:]     = bump(num_x,x_edges)[0]
    disc_source_output     = disc_source(num_x,x_edges)
    psi0_edges[reg4,:]     = disc_source_output[0]
    source_edges[reg4,:]   = disc_source_output[3]    
    ...
    for l in range(num_epochs):
        ...
        exact = timestepping(psi0, 0, 0, params, 
                sigs, sigt, N_exact, source, batch_size,device)[0]
        psi  = timestepping(psi0, 1, NN_model, params,
                sigs,sigt,N, source, batch_size,device)[0]
