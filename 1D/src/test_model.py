...
def testing(params):
    ...
    with torch.no_grad():
        ...
        FPN,sigf      = timestepping(psi0, 1, NN_model, params, sigs, sigt, N, source,batch_size,device)


        error0     = torch.sqrt(obj_func(PN  - exact[:,:,0:N+1])/obj_func(exact[:,:,0:N+1]))
        errorf     = torch.sqrt(obj_func(FPN -  exact[:,:,0:N+1])/obj_func(exact[:,:,0:N+1]))
