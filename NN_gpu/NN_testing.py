...
def initial_data_disc_cs(N,num_x,num_mu,glw,mu,x):
    z      = torch.zeros([num_x,num_mu])
    sigs   = torch.zeros(num_x)  
    source = torch.zeros([num_t+1,num_x,N+1])
    z_leg  = torch.zeros([num_x,N+1])
    ...
    for m in range(0,num_x):
        if x[m]>= -0.65 and x[m] <= -0.35 or x[m]>= 0.35 and x[m]<= 0.65: 
            sigs[m] = 0.02
        else:
            sigs[m] = 1
...
def testing(params,mesh,funcs):
    ...
    if IC_idx == 0:
        exact0,sigs,sigt,source_exact,CN = initial_data_disc_cs(N_exact,num_x,num_mu,glw,mu,x)
    elif IC_idx ==1:
        exact0,sigs,sigt,source_exact,CN = initial_data_vanishing(N_exact,num_x,num_mu,glw,mu,x)
    elif IC_idx == 2:
        exact0,sigs,sigt,source_exact,CN = initial_data_linesource(N_exact,num_x,num_mu,glw,mu,x)
    ...
    if IC_idx == 0:
        y0, sigs, sigt,source,CN     = initial_data_disc_cs(N,num_x,num_mu,glw,mu,x)
    elif IC_idx == 1:
        y0, sigs, sigt,source,CN     = initial_data_vanishing(N,num_x,num_mu,glw,mu,x)
    elif IC_idx == 2:  
        y0, sigs, sigt,source,CN     = initial_data_linesource(N,num_x,num_mu,glw,mu,x)
