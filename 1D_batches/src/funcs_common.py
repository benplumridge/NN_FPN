...
def PN_update(
    ...
    y_update_out = torch.zeros([batch_size, num_x, N + 1], device=device)
    sigt_y    = sigt * y_expand[:, 1 : num_x + 1, :]
    y_update  = A_Dy - sigt_y
    y_update[:, :, 0] = (
            y_update[:, :, 0] + sigs[:, :, 0] * y_prev[:, :, 0] + source
    )
    if filter_type == 0:
        y_update_out = y_update
    if filter_type in (1,2,3):
        filter_order = params["filter_order"]
        filt_input = torch.arange(0, N + 1, 1) / (N + 1)
        filter     = -torch.log(filter_func(filt_input, filter_order))
        y_update_out = y_update - sigf[:, :, None] * y_prev * filter   
    return y_update_out, sigf
