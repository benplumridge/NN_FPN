...
class SimpleNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.relu(self.output(x))                     #output layer 
        return x
...
def upwind_flux(N,num_basis,psi,params) :
    ...
    f_plus  = torch.zeros([batch_size,num_y,num_x,num_basis], dtype=torch.float32)
    f_minus = torch.zeros([batch_size,num_y,num_x,num_basis], dtype=torch.float32)
    g_plus  = torch.zeros([batch_size,num_y,num_x,num_basis], dtype=torch.float32)
    g_minus = torch.zeros([batch_size,num_y,num_x,num_basis], dtype=torch.float32)

    dx_left  = torch.zeros([batch_size,num_y,num_x,num_basis], dtype=torch.float32)
    dx_right = torch.zeros([batch_size,num_y,num_x,num_basis], dtype=torch.float32)
    dy_up    = torch.zeros([batch_size,num_y,num_x,num_basis], dtype=torch.float32)
    dy_down  = torch.zeros([batch_size,num_y,num_x,num_basis], dtype=torch.float32)
    ...
    #     source_y_index  = params['source_y_index']   
    #     dx_left[:,source_y_index,1,0] = (psi[:,source_y_index,1,0] - source_boundary)/dx
    ...
    f_plus[:,:,-1,:]  = torch.zeros(batch_size,num_x,num_basis)
    f_minus[:,:,0,:]  = torch.zeros(batch_size,num_x,num_basis)
    g_plus[:,-1,:,:]  = torch.zeros(batch_size,num_x,num_basis)
    g_minus[:,0,:,:]  = torch.zeros(batch_size,num_x,num_basis)
        
    fluxes  = f_plus + f_minus + g_plus + g_minus

    #partial derivatives for model features
    dx_psi = torch.zeros([batch_size,num_y,num_x,num_basis], dtype=torch.float32)
    dy_psi = torch.zeros([batch_size,num_y,num_x,num_basis], dtype=torch.float32)
...
def preprocess_features(N,psi, dxpsi, dypsi, scattering, source, params, psi0_mean):
    Lx = params['Lx']
    Ly = params['Ly']
    num_x = params['num_x']
    num_y = params['num_y']
    batch_size = params['batch_size']
    psi_norms  = torch.zeros([batch_size,num_y,num_x,N+1])
    dpsi_norms = torch.zeros([batch_size,num_y,num_x,N+1])
    ...
    psi_in        = LxLy*psi_norms/psi0_mean[:,None,None,None]
    dpsi_in       = LxLy*dpsi_norms/psi0_mean[:,None,None,None]
    scattering_in = LxLy*scattering_in/psi0_mean[:,None,None,None]
    source_in     = source_in/psi0_mean[:,None,None,None]
...
def timestepping(psi0, filt_switch, NN_model, params, sigs, sigt, N,num_basis, source):
    ...
    psi_prev = torch.zeros([batch_size,num_y,num_x,num_basis],dtype = torch.float32)
    psi_prev[:,:,:,0] = psi0

    #psi0_mean = 10*torch.mean(psi0,dim = (1,2))
    psi0_mean = torch.ones(1)

    for k in range(1, num_t + 1):
        psi1_update       = PN_update(psi_prev,N,params,num_basis,sigt,sigs,filt_switch,source,NN_model,psi0_mean)[0]
        psi1 = psi_prev + dt*psi1_update 
        psi2_update, sigf = PN_update(psi1,N,params,num_basis,sigt,sigs,filt_switch,source,NN_model,psi0_mean)  
        psi = psi_prev + 0.5*dt*(psi1_update + psi2_update)
        psi_prev = psi
    return psi_prev[:,:,:,0], sigf[0,:,:]

def PN_update(psi_prev,N,params,num_basis,sigt,sigs,filt_switch,source,NN_model,psi0_mean): 
    ...
    sigf       = torch.zeros([batch_size,num_y,num_x],dtype = torch.float32)
    psi_update = torch.zeros([batch_size,num_y,num_x,num_basis],dtype = torch.float32)
    inputs     = torch.zeros([batch_size,num_y,num_x,num_features],dtype = torch.float32)

    if filt_switch == 1:
        inputs =  preprocess_features(N,sigt_psi,A_dxpsi,A_dypsi,scattering,source,params,psi0_mean)     
    ...
    # else:
    #     source_y_index  = params['source_y_index'] 
    #     psi_update[:,0:source_y_index.start,0,:] = psi_update[:,0:source_y_index.start,1,:]
    #     psi_update[:,source_y_index.stop+1,0,:]   = psi_update[:,source_y_index.stop+1,1,:]
...
def compute_cell_average(f,num_x,num_y,batch_size):
    average = torch.zeros(batch_size,num_x,num_y,dtype = torch.float32)
